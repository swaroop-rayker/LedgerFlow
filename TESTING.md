...
| D7 | P4 | **A lost folder is asked for, not failed into** | Revoke the backup folder's access (Settings → Apps → LedgerFlow → Storage, or delete the folder), then open Back up now. | The screen offers "Choose folder" before any words are typed. Nothing was written and no error dialog appears. (There is no background backup to fail silently: §5.9's nightly worker was replaced by the manual backup, §16 Q23.) |
