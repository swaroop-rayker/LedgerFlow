...
| D10 | P4 **[owner]** | **Restore onto a clean install** (ADR-0026, §16 Q11) | Needs an install with **no vault** — a guest or secondary user on the phone, never the live one (restore refuses to run over a vault, and clearing the live one's data is exactly what BUG1 forbids). Install the debug APK for that user. On onboarding's first screen tap **Restore from a backup** → **Choose backup folder** → the folder D6 wrote to → keep the newest backup selected → type the 24 words → **Restore**. Then repeat once with a *wrong* valid phrase and once with **Use a single file**. | The report names the rows restored and, as a number, any receipt images not found; **Open my ledger** lands on the restored data. More shows the backup date and "Back up now" does not ask for a folder. Force-stop and relaunch: opens with no prompt. The wrong phrase says "not the one this backup was made with … Nothing was restored", and a relaunch after it is still onboarding. The single file restores the rows and says every receipt image wasn't found alongside it. |
