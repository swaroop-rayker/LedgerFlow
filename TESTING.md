...
**One exception, owner-approved (2026-09-19):** D10 needs an install with no
vault, and this phone reports `pm get-max-users` = 1 — no guest or secondary
user, and Secure Folder is closed to `adb`. So D10 runs on the `playSafe`
build, which is a separate app with its own empty data beside the live one. It
covers the restore path, which is shared code; it does not stand in for the
deferred `playSafe` rows above.
...
| A6 | P1 | **A migration does not shrink what the app shows** (BUG20's shape) | On a vault with entries spanning several months *and* at least one entry older than the oldest `daily_rollup` row, install a build carrying a schema migration over it. Then compare, for the same range and the same book: the Ledger's entry count, Analytics' total and transaction count, and the Export's row count. | **All three agree, before and after.** BUG20 was analytics silently omitting every entry older than the rollup — the ledger was right, the chart was short, and nothing failed. A migration is the event most likely to reintroduce that, because it is when derived tables get rebuilt. Do the comparison on the **same** range with the **same** filters; "looks about the same" is not this test. If the three disagree, the number to trust is the Ledger's — it reads base tables — and the defect is in whatever derives from them. |
...
| D5 | P1 **[owner]** | **Recovery Kit warning is honest** | On the Recovery Kit step, choose "Save as text file" (the secondary option). Then repeat with the main button, **"Save as PDF"** (from 2026-09-26 the default, so a new user's kit carries the QR). Open the PDF: the words, then "How to restore", then **"Scan to restore" with the QR code below it**, with no text touching or under the code (BUG36), and nothing running off the page edge. | The dialog says the file is plaintext, says what it grants, and says where it is going, *before* the picker opens (D-07). Then open the saved file and confirm it contains what the dialog said it would. **PDF only:** it also carries a QR code (ADR-0028) — print or display it, scan it from the Recovery or Back up now screen, and the 24 words fill in. The `.txt` kit has no code. **An install that onboarded before 2026-09-20 has no QR kit:** make one from **More → Back up now**: after a successful backup, tap **Save Recovery Kit**. The same warning appears, now with the QR sentence, then the file picker. The screen then says "Recovery Kit saved"; "Not now" makes the offer disappear (ADR-0028, amended 2026-09-26). **The scanner itself, no real words needed:** show a foreign QR (e.g. a Wi-Fi code) on another screen, then a QR of `LFBK1:` plus the public test phrase (`abandon` ×23 then `art`). The first must leave the camera open. The second must close it with all 24 words filled, and the button then reads "Back up" (BUG34). Leave the screen without backing up. **Layout** (BUG35): the scanner's card, its instruction line and **Type the words** sit above the navigation bar, with nothing overlapping. Measure with `uiautomator dump` against the nav bar's frame (2026-09-26: button bottom 1949, nav bar from 2196). |
| D6 | P4 **[owner]** | **"Back up now" writes a backup that opens** (§16 Q23) | More → Back up now. Choose a folder on **local storage** (e.g. Documents/LedgerFlow). Type the 24 words, tap Back up. Then repeat with a folder on a **cloud provider** (Google Drive). | Each time: "Backed up to ledgerflow-YYYYMMDD-HHMMSS.lfbk", and that file is in the folder alongside an `attachments/` folder holding one `.lfba` per receipt image. The Settings row now shows today's date. **This row is the only check of `SafBackupFolder` against a real document provider** — its logic is tested against a directory, but a real tree grant only comes from the system picker. On the cloud provider, watch for a failure message: a provider that refuses fsync is handled, one that refuses rename falls back to a direct write, and either would show up here first. |
| D6b | P4 **[owner]** | **The wrong words write nothing** | Back up now with a *different* valid phrase (e.g. an old install's Recovery Kit). | "Those words are a valid phrase, but not the one for this phone's data. Nothing was backed up." No new file in the folder; the Settings date does not change. |
| D6c | P4 **[owner]** | **Five kept, nothing else touched** | With a few unrelated files in the backup folder, run Back up now six times. | Five `ledgerflow-*.lfbk` remain, the newest five. The unrelated files are untouched. |
| D7 | P4 | **A lost folder is asked for, not failed into** (BUG27) | Rename or delete the backup folder in the Files app (Android has no settings screen that revokes one folder's grant; the grant surviving a rename is exactly what BUG27 was), then open Back up now. Then choose a folder, and use **Change folder** to choose a different one. | The screen offers "Choose folder" before any words are typed; nothing was written and no error dialog appears. With a folder chosen it reads "Backups go to …"; after Change it names the new folder and says earlier backups stay in the previous one. (The nightly pass (ADR-0027) meets a lost folder too, and does not fail into it either: it records a `NoFolder` skip in `app_meta`, writes nothing and says nothing — a skip is not a failure, so "Back up now" shows no line for it. What surfaces a lost folder is this screen asking for one, and Home's reminder once the last backup is more than 7 days old, D11.) |
| D10 | P4 **[owner]** | **Restore onto a clean install** (ADR-0026, §16 Q11) | Needs an install with **no vault** — a guest or secondary user on the phone, never the live one (restore refuses to run over a vault, and clearing the live one's data is exactly what BUG1 forbids). Install the debug APK for that user. **A phone without multi-user** (`pm get-max-users` reports 1 — the owner's Samsung, 2026-09-19; Secure Folder is not reachable over adb): install the **other flavour** instead (`installPlaySafeDebug`, package `com.ledgerflow.playsafe.debug`), which is a separate app with its own empty data beside the live `smsFull` install, and reaches the same backup folder. Between cases, `pm clear` **that package only**. The restore code is shared, so this covers restore on the `playSafe` build rather than `smsFull`; say so in the run record. **The restoring install must be at least as new as the build that wrote the backup:** a nightly `.lfbk` is container v2 (ADR-0027, `16f79dc`, 2026-09-20), and an older build correctly refuses it with "made by a newer version of LedgerFlow … Nothing was restored". That happened on 2026-09-25 with a playSafe copy installed on 09-19, so run `installPlaySafeDebug` before this row, not only once. **A restored copy inherits enrolment and the backup folder.** A restore imports every `app_meta` row, including the nightly sealing key, and records the folder chosen during the restore. So the copy starts its own 03:00 pass into the **same folder**. Rotation keeps the newest five files whoever wrote them, so the copy's backups would push out the real install's. When the row is done, `pm clear` the copy (or point it at another folder via Back up now → Change folder) **before 03:00**. On onboarding's first screen tap **Restore from a backup** → **Choose backup folder** → the folder D6 wrote to → keep the newest backup selected → type the 24 words → **Restore**. Then repeat once with a *wrong* valid phrase (the public BIP-39 test phrase — `abandon` ×23 then `art` — is valid and nobody's key)**, once by scanning the Recovery Kit's QR instead of typing** (ADR-0028: the words fill in, and a QR from anywhere else leaves the camera open) and once with **Use a single file**, which opens Android's file picker; tapping a backup in the app's own list is the folder route. With the fixes for BUG29 and BUG30, back with the keyboard open only closes the keyboard, and the screen starts empty each time it is opened. | The report names the rows restored and, as a number, any receipt images not found; **Open my ledger** lands on the restored data. More shows the backup date and "Back up now" does not ask for a folder. Force-stop and relaunch: opens with no prompt. The wrong phrase says "not the one this backup was made with … Nothing was restored", and a relaunch after it is still onboarding. The single file restores the rows and says every receipt image wasn't found alongside it. |
| D12 | P4 **[owner]** | **A backup appears overnight, with nothing typed** (ADR-0027) | After one manual backup (which enrols), leave the phone overnight with the backup folder reachable. **The evening before, confirm the pass is aimed at 03:00** (ADR-0027, amended 2026-09-23 — as first built it drifted to the hour of its last run, BUG31): `adb shell dumpsys jobscheduler` and find the `SystemJobService` job whose extras name the unique work `nightly-backup-0300`; its earliest run time is the next 03:00 local, as a delay from now. WorkManager's own record says the same without parsing that — pull `no_backup/androidx.work.workdb` with `adb exec-out run-as <package> cat …` (never plain `adb shell cat`, which mangles binary; it holds no ledger data) and read `next_schedule_time_override` on the `WorkSpec` for that name. Next morning, list the folder. Then check the More row. | A new `ledgerflow-*.lfbk` **written at or after 03:00 local** and before you picked the phone up, judged by the file's **modified time** (`ls -la`), not by its name. 03:00 is the earliest the pass may start and Doze decides the rest, so 05:40 is a pass and 22:00 the evening before is BUG31. **The name's stamp is UTC** (`fileNameFor`, deliberately, so names sort the same across zones and DST). In IST a correct pass between 03:00 and 05:30 is therefore *named* between about 21:30 and 00:00 **of the previous day**: `ledgerflow-20260923-221909.lfbk` was written at 03:49 IST on the 24th. Read by its name, that looks like BUG31, and it isn't, with no words typed for it, and `attachments/` holding a `.lfba` per receipt image. The More row reads "Backs up nightly on its own". **Then restore that file** on the playSafe install (D10's route, with the playSafe build brought up to date first and the copy cleared afterwards — both in D10): it must open with the 24 words, which is the only proof the nightly file is good — the phone itself cannot check that. **If no backup appeared**, open Back up now: a failed automatic pass says so there, naming the date (ADR-0027 amended). Nothing on that screen means the pass never ran — check the schedule as above. A pass aimed at tomorrow's 03:00 means the night's pass did run and re-aimed itself; a pass aimed at some other hour means the re-aim did not happen, and the next pass should correct it. |
| D13 | P4 | **A nightly pass with nothing to do is quiet** | Before any manual backup (a fresh install — the playSafe copy works, being installed and not enrolled), leave it overnight. The pass is aimed at 03:00 like D12's, so it is the next morning's check, not the same evening's. | No file, no notification, no error anywhere. A pass with no enrolment, no folder or no open vault records a reason and says nothing (ADR-0027). |
| D11 | P4 | **The backup reminder appears, and a backup clears it** (BUG4(c) as amended) | Leave the phone more than 7 days after the last backup (the date is inside the encrypted vault, so it cannot be backdated honestly — this row waits for real time). Open Home. Then back up (an **[owner]** step: it needs the 24 words) and return to Home. | Home shows "Last backup N days ago. Anything since is only on this phone." with **Back up now**, which opens the Back up now screen. After the backup the line is gone without restarting the app. On a day within 7 days of a backup, Home shows no line at all. |
...
| D9 | P4 **[owner]** | **The keyboard does not learn the phrase** (BUG25) | Type all 24 words on the Back up now screen (or Recovery). Then open any notes app and type the first two letters of an unusual word from your phrase. | The keyboard does **not** suggest it, and did not show suggestions while the words were being typed. If it does, the keyboard ignores the password input type; note which keyboard, because that is outside what the app controls (§8 BUG25). |
...
| F19 | **P2-8** — 🕕 **pre-release matrix** (owner's call, P2-9) | **The banner appears after six hours dead, and clears on reconnect** | With access granted, kill the listener and leave it killed — the reliable way is the OEM battery optimiser (Settings → Battery → restrict LedgerFlow), not force-stop, which Android treats differently. After six hours, Home shows **"Notification capture has stopped"**, *not* "is off": the two sentences are for different causes and swapping them sends the user to the wrong fix. Lift the restriction; the banner clears on the next resume. **What is already covered, so you know what this row adds:** the > 6 h rule is pinned at the millisecond boundary by `ListenerHealthEvaluationTest`, the DEAD banner's exact wording by `DashboardBannerContentTest`, and "clears on reconnect" was observed on device via the NOT_GRANTED banner. What no test can produce is a *real* six-hour outage on hardware — that is the whole of what remains, and it is why this row moved to the pre-release matrix rather than blocking P2-8. It sits beside BUG2's OTA survival, which has the same character: no runner can simulate it and no amount of unit testing substitutes for it. |
