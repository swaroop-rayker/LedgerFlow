...
| D12 | P4 **[owner]** | **A backup appears overnight, with nothing typed** (ADR-0027) | After one manual backup (which enrols), leave the phone overnight with the backup folder reachable. Next morning, list the folder. Then check the More row. | A new `ledgerflow-*.lfbk` dated overnight, with no words typed for it, and `attachments/` holding a `.lfba` per receipt image. The More row reads "Backs up nightly on its own". **Then restore that file** on the playSafe install (D10's route): it must open with the 24 words, which is the only proof the nightly file is good — the phone itself cannot check that. **If no backup appeared**, open Back up now: a failed automatic pass says so there, naming the date (ADR-0027 amended). Nothing on that screen means the pass never ran — check the schedule with `adb shell dumpsys jobscheduler \| grep NightlyBackupWorker`. |
| D13 | P4 | **A nightly pass with nothing to do is quiet** | Before any manual backup (a fresh install), leave it overnight. | No file, no notification, no error anywhere. A pass with no enrolment, no folder or no open vault records a reason and says nothing (ADR-0027). |
