...
| **P2 — Automated ingest** | Shared rule engine, `ParseIngestWorker`, cross-source dedupe, Inbox, notification actions, approve/discard — **plus both capture adapters: SMS receiver (`smsFull`) and `NotificationIngestService` (both flavours)**. Permission UX, listener rebind and the §5.2 health banner shipped at P2-8. | 50-SMS + 50-notification golden corpus passing. Dedupe test: same UPI txn via both sources → exactly one pending row. **Both outstanding at P2-9, and the corpus is the hard half**: 7 of the 7 notification fixtures are synthetic, and §16 Q15 is what synthetic fixtures cost — every SMS fixture supplied a sender the allowlist had been written against, so the allowlist matched no real bank for three steps with every test green. Notification access is now granted and the listener bound, so a real capture is finally possible. |
