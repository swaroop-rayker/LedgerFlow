...
14. **An allowlist change does not re-triage messages it previously rejected.** `triageCapturedSms` only considers rows still at `CAPTURED`; once a row is marked `SENDER_NOT_ALLOWLISTED` nothing ever revisits it. That made the v1 sender-pattern defect (see below) permanent for every message received before the fix, including the owner's own test payment — the fix makes the *next* bank SMS work and cannot recover the last one.

    It is not only a migration problem. §5.1 ships a user-editable allowlist and P5 adds a Settings editor for it, so the same dead end is reachable by ordinary use: a user notices their bank was missing, adds it, and every message already received stays invisible with nothing explaining why. D-09 keeps the raw body for 90 days precisely so a message stays "replayable against a later ruleset" (Q1), so the data to replay is deliberately there and only the trigger is missing.

    Not built at the time of the fix because it makes a terminal status non-terminal, which is a semantic change inside a §7 danger zone and wants a decision rather than an inference. The shape is small: re-triage `SENDER_NOT_ALLOWLISTED` rows whose body survives, on allowlist change rather than on every worker pass. Open questions are what triggers it (a seed-version bump, a Settings edit, or both) and whether a re-admitted message keeps its original `received_at` for the dedupe window — it should, but that is exactly the kind of thing that needs saying out loud before it is coded.
