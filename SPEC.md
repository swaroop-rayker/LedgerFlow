...
- **An A4 GST invoice is read as a table** (`ReceiptTable`). Quick-commerce
  ...
  **A PDF whose text names no shop** (bigbasket draws its supplier block, so
  ...
  cell holding a name that ends in `Pvt Ltd` / `Private Limited` / `Limited` /
  `LLP`, cut after the suffix (`MerchantFallback`, owner's decision 2026-09-19).
  Only on that path: on a photograph the brand is usually the large name, and
  type size still decides there. Measured on the device: bigbasket reads
  `Innovative Retail Concepts Pvt Ltd`, graded by `ReceiptCorpusGradingTest`.
...
- **Merchants:** canonical merchant + alias table. SMS `merchantRaw` and OCR headers normalize (uppercase, strip punctuation/legal suffixes/store codes) then fuzzy-match (Jaro-Winkler ≥ 0.88, `JaroWinkler.MERCHANT_THRESHOLD` in `:core:domain` — one implementation, three callers, so the number cannot drift between them) against aliases. Unmatched → a new merchant is **created** via `createOrGet` rather than the entry being refused (§5.1); the fuzzy match is a merge *suggestion* at review time, not a gate. The user can merge merchants later, which is the cheap correction — a wrong auto-merge is the expensive one, which is why the threshold only ever suggests. **Aliases are learned, and exact (item 7b, owner, 2026-09-19).** Until then the `merchant_alias` table existed and nothing wrote or read it. Now: approving a candidate under a merchant the user chose — picked or typed — records what the SMS or receipt *read* as an alias of that merchant, after the approval commits and never before; the next "GEDDIT CONVENIENCE PRIVATE LIMITED" opens the review on Zepto and files there. `findByName` and `createOrGet` resolve a taught name after the merchant's own name and **before** a hidden merchant's name is un-hidden (BUG28). Only exact normalised matches: the fuzzy match above stays a suggestion. A live merchant's own name is never taken as another's alias; a name taught again moves its alias; an alias to a hidden merchant resolves to nothing. A merge carries the folded merchant's aliases to the survivor and records its name as one.
...
| **Two-book parallel view** | Debit and credit on mirrored axes, visibly separate, never a net line. **One shared scale**, so an asymmetric month looks asymmetric rather than being flattered by per-half scaling — the net figure's false impression reintroduced as geometry. Both halves carry the same positive axis label; a negative one below the line would invite reading the two as one signed series. Built from **two ledger-bound reads**, and the one section that ignores the filters, since §5.5's category trees are disjoint per book | **P3** |
...
**Amount, period, start date and rollover are all editable after creation; the category pair is not** (Q20). The category and subcategory are what the budget *is* — the repository enforces one live budget per pair — so changing them is creating a different budget. The other four are ordinary settings and go in one statement, because amount and period decide the same figure and a half-applied pair would let `BudgetAlertWorker` announce a threshold against a budget that never existed.

**Editing the period or the start date re-cuts the period in flight**, and that is deliberate rather than tolerated. A period is *derived* from `start_date` and the period length rather than stored, so there is no "this period" record that could be preserved; the user asked for a different window and is shown it, spend-so-far included. It also makes the alert bookkeeping fall out for free: `thresholdToAnnounce` suppresses on `alertPeriodStart == periodStart`, so a re-cut window re-announces a crossing by itself and an amount-only edit stays silent — no reset, no extra column, and both directions pinned by tests.
...
to it once a night, aimed at 03:00, battery-not-low, with **no phrase anywhere in the process**.
...
**When it runs: aimed at 03:00, decided by Android** (ADR-0027, amended
2026-09-23; §8 BUG31). The schedule is a one-day periodic request whose next run
every pass first points at the next 03:00 local, through WorkManager's
`setNextScheduleTimeOverride`. Without that, a periodic request counts its day
from the last run, and as first built the pass drifted to about 21:50 on the
owner's phone. 03:00 is the **earliest** the pass may start. Doze holds it to a
maintenance window, app standby can defer it, and a low battery skips the night.
Picking the phone up ends Doze, so it lands between 03:00 and the morning.
**The screens say "nightly" and never name the hour**, because the hour is an
aim and "nightly" is the promise. Where DST removes 03:00 the pass aims at the
end of the gap, and where DST repeats it the first one counts, so one night
never writes two backups. A pass that dies before it re-aims costs one drifted
night, never the schedule. Existing installs move to the aimed schedule (unique
name `nightly-backup-0300`) on their next cold start, which cancels the old
`nightly-backup`.
...
**Restore, as shipped (ADR-0026, closes §16 Q11).** At first run only, from a quiet "Restore from a backup" link on onboarding's first screen. The user picks the backup *folder* (the newest `ledgerflow-*.lfbk` preselected; "Use a single file" for one that travelled alone) and types that backup's 24 words. **The backup is decrypted and parsed in memory before anything is written**, so wrong words, a damaged file and a newer schema each leave no wrap, no database and no marker. Then a marker file (`filesDir/<db>.restore-pending`, fsynced), then **a fresh DEK wrapped under the backup's own phrase** — the user keeps the one phrase they hold — then every row in one transaction with two `app_meta` corrections inside it (this build's `schemaVersion`; this install's `backupTreeUri`, or none), then the images (ADR-0023), then the marker is removed. The vault opens only after the user has read the report, which names the rows and, as a count, any receipt images that did not come back. **Restore refuses to run over an existing vault**; it replaces onboarding, never data. While the marker exists, launch routes back to the restore screen and nothing — background capture included — opens the vault.
...
**A kit can also be saved after onboarding, from "Back up now"** (ADR-0028,
amended 2026-09-26). Onboarding was the only place a kit could be written, so an
install that onboarded before kits carried a QR code (the owner's did) could
never scan anything. A successful backup has just proved the typed words are
this vault's phrase; the screen then offers **"Save Recovery Kit"** — a PDF with
the QR code — behind the same D-07 dialog. The words are held out of the UI
state, only until the kit is saved, "Not now" is tapped, the words are changed,
another backup starts, or the screen closes (BUG30's rule); a failed write keeps
them so another place can be tried. No new screen ever asks for the phrase.

**The scanner** is a full-screen camera with one card over it: a line saying
what to point at, and **"Type the words"**, which returns to the field. The card
is inset for the system bars and cutout, because the scanner replaces the
screen's `LfScaffold` (§8 BUG35). It hands every *distinct* code to the screen
once; a foreign code is answered "keep looking" and the camera stays open for the
next one (§8 BUG34).

The file is **not** encrypted, and the tap that writes it is gated behind a dialog that says so in those terms — that this file is the master key to every backup, that it is being written to shared storage which may be cloud-synced, and where it is going — and, for the PDF, that it carries a QR code a camera can read, so a photo of the page is as good as the words (ADR-0028 rule 5; the dialog lacked this sentence until 2026-09-26, and is now one shared composable, `LfRecoveryKitWarningDialog`, used by onboarding and "Back up now"). The rejected alternative was a password-protected PDF: it reintroduces a user-chosen secret into the recovery path, which is exactly what D-03 forbids for the backup path and for the same reason, and PDF password encryption is weak on its own terms. Encrypting the kit also creates a regress — the thing that protects the thing that protects everything — whose answer is always another secret the user can forget. Plaintext plus informed consent is the honest version of a trade-off that cannot be engineered away.
...
3. ❌ Neither factor available:
   → offer restore from .lfbk backup file (needs the phrase)
     — built for first run (no wrap, no database) as onboarding's
       "Restore from a backup" branch, ADR-0026; not yet offered from Recovery
...
**Restoring a backup is the one alternative to the gate** (ADR-0026). It is reached from the first step and replaces all five: the currency comes from the backup, and typing all 24 words — which must open the chosen backup before anything is written — is stronger proof of the phrase than the three-word challenge. The vault is then protected by that phrase, not a new one.
...
| **BUG4** — backup errors / undecryptable restore | Keystore key not portable; corrupt/truncated backup; silent worker failure | (a) §7.2 dual-wrap + §7.5 own backup format. (b) Every `.lfbk` write is atomic: write to `.tmp` → fsync → verify by full decrypt-and-parse → rename. A backup is only recorded as successful after a **round-trip verification**. (c) ~~`BackupWorker` failure posts a persistent notification; `lastBackupAt` age > 7 days shows a Dashboard warning banner.~~ **Amended by ADR-0025:** there is no `BackupWorker` — backups are manual — so there is no background failure to announce. A failed manual backup says so on screen and changes nothing; the More row shows the last *verified* backup's date, or "No backup yet. Your data exists only on this phone." **The Dashboard reminder (owner, 2026-09-19), built:** one compact line at the top of Home, with a "Back up now" action, whenever there is no backup or the last verified one is more than **7 days** old — "No backup yet. Your data exists only on this phone." or "Last backup N days ago. Anything since is only on this phone." Not dismissible while true; a backup clears it without a resume, and each resume re-judges the same date, because it grows stale with nothing emitting. Nothing is shown before the date has been read, so a first frame never claims "No backup yet" to someone who has one. `BackupReminder.of` is the rule (`BackupReminderTest`), `DashboardBackupReminderTest` the screen's side, `DashboardBackupReminderContentTest` the rendered words and the action at font scale 2.0. (d) Keep the last **5** backups, rotate oldest. | Instrumented: backup → wipe → restore → assert full row-level equality across all tables. Runs in CI on every PR. |
...
| **BUG20** — analytics silently omitted every entry approved before the rollup existed | `MIGRATION_8_9` creates `daily_rollup` empty and left the filling to ADR-0006's nightly reconciliation, whose comment said "the first reconciliation fills it". That pass requires the device to be **idle and charging** — right for repairing drift, wrong for a cold cache, because until it runs every figure on the Analytics screen omits every entry older than the migration. On the owner's device the constraints had never once been met, so the incremental path had kept every *later* entry correct and the screen looked plausible while missing the older half: two real credits of ₹6,300 and ₹250, plainly visible in the Ledger, reported by D1 as "In ₹0.00". **Reported by the owner, not by a test** — every test seeds its own data *after* the migration, so no fixture could ever have been in the state that breaks. | `RollupWorker.fillIfCold`, a one-shot with **no constraints**, enqueued on every cold start under `KEEP`. It calls the same recompute — ADR-0006's "one routine" is untouched; what is new is the *decision* about when to run it, gated on `rollupLastReconciledAt` being absent so it is a metadata read forever after the first run. Gating on the stamp rather than on emptiness matters: an empty table is legitimate when the ledger is empty, and emptiness would trigger a full recompute on every launch. | `backfill_fillsARollupThatHasNeverBeenBuilt` reproduces the state the migration leaves — rows deleted *and* the stamp cleared — and `backfill_doesNothingOnceReconciliationHasRun` pins the no-op, corrupting the table afterwards to prove the cold fill declines to repair drift, which is the nightly pass's job. Disabling the fill reddens the first and only the first. |
...
| **BUG25** — the recovery phrase could be learned by the keyboard | Found while scoping a manual "Back up now" (§16 Q23), which would reuse the same field. Onboarding's word challenge and the Recovery screen took the 24 words in a plain `LfTextField` with default keyboard options: autocorrect on, no password variation. Gboard and Samsung Keyboard learn typed words into a personal dictionary and, with sync on, carry it off the device — so the one secret that protects every `.lfbk` could end up stored outside the app, in a place §7.2's threat model never accounted for. Nothing on screen distinguishes a field that allows this from one that does not. | `LfKeyboards.RecoveryWord` in `:core:designsystem`: `KeyboardType.Password` (which mainstream keyboards neither learn from nor suggest into) with no masking transformation, so the words stay visible; autocorrect off; no capitalisation. Every phrase field uses it. **A limit, stated:** Android's explicit `IME_FLAG_NO_PERSONALIZED_LEARNING` is not reachable through Compose's `KeyboardOptions`, so this relies on keyboards honouring the password variation, the convention password managers depend on. | `Bug25_PhraseFieldsDoNotTeachTheKeyboardTest` (source guard: every `LfTextField` in the phrase screens passes the option; removing it from the Recovery screen reddens it) and `LfKeyboardsTest` (the option's three properties). A source guard because what changes is the `EditorInfo` handed to the keyboard, which no screenshot or semantics node shows. |
| **BUG26** — a backup that authenticated and then did not parse would have crashed the restore | Found while building the first-run restore (ADR-0026). `DatabaseBackupManager.restore` decoded the payload JSON outside the `runCatching` that guards the import, so a `.lfbk` whose GCM tag verified but whose payload this build could not read — a writer bug, or a format it does not understand — escaped as an exception rather than a result. Unreachable while restore had no caller; the restore screen would have been its first. | Decrypt-and-parse is split out as `DatabaseBackupManager.open`, which reports a parse failure as `LfbkFailure.Malformed` — shown to the user as a damaged backup, with nothing written — and the import takes only what `open` produced (`OpenedBackup.Ready`, whose constructor is internal). | `Bug26_UnparseablePayloadIsReportedNotThrownTest` (JVM): an authentic unparseable payload is `Malformed`; a well-formed one still opens. |
| **BUG27** — a backup folder that no longer existed was failed into, not asked for | Found on the owner's phone running `TESTING.md` D7 (2026-09-19): the backup folder was renamed in the Files app and "Back up now" still went straight to the 24 words. The check was whether the app *held a grant* for the folder, and renaming or deleting a folder leaves its grant in place, so the backup would have failed at the write with a sentence about disk space. The same gap meant there was no way to change folders at all — the picker appeared only when no folder was known — so moving backups to a cloud drive required losing the current folder first. | (a) `BackupFolder.displayName()`: the folder's own name, or null if it is gone — a query on the SAF tree's root document, a directory check in tests. `DefaultBackupRepository` treats a folder as chosen only if the grant is held **and** it answers; `backupFolderName()` replaces `hasBackupFolder()`. (b) "Back up now" names the folder ("Backups go to …") with an inline **Change folder**; after a change it says earlier backups stay in the previous folder. (c) Choosing a new folder releases the replaced folder's grant — Android caps the grants an app may hold. **Not** released when a check merely fails: that can be transient (a cloud provider offline), and dropping a good grant would force the user to choose it again. (d) A backup that finds no folder re-asks rather than trusting what the screen was told when it opened. | `Bug27_LostFolderIsAskedForTest` (instrumented: a deleted folder reads as none and a backup writes nothing; a change releases only the replaced grant) and `BackupNowViewModelTest` (`bug27_changingTheFolder_…`, `aLostFolder_asksAgainAndKeepsTheWords`, `aFirstFolder_isNotReportedAsAChange`). Mutation-swept: trusting the grant alone, releasing on a re-choice of the same folder, never reporting a change, and not re-asking after a lost folder each redden their test. Verified on the owner's phone against the real SAF provider: with the folder renamed, the screen asks for one before any words. |
| **BUG28** — a merchant merge was undone by the next message naming the merged merchant | Found while building item 7b (2026-09-19), proved with a failing test before any change. Merging "Geddit Convenience" into "Zepto" soft-deletes Geddit. The next SMS or receipt naming Geddit reached `createOrGet`, whose BUG11 path — a name held by a *hidden* merchant un-hides it rather than failing the unique key — restored Geddit, and every later entry filed there again: the merge quietly reversed, entry by entry, with nothing on screen to say so. | A merge now records the merged merchant's name as an **alias** of the survivor and moves its taught aliases across (item 7b), and `createOrGet` resolves an alias **before** its un-hide path. BUG11's behaviour is unchanged for a merchant that was hidden rather than merged. | `Bug28_MergeIsNotUndoneByTheNextCaptureTest` (instrumented): after a merge, the merged name resolves to the survivor and the merged merchant stays hidden. Red on the unfixed code (the old merchant came back); red again when the merge's alias is removed or when the alias lookup is moved after the un-hide. |
| **BUG29** — back with the keyboard open left the restore screen, and the 24 words on it | Found on the owner's phone running `TESTING.md` D10 (2026-09-19): after a refused restore, one back press to close the keyboard went to onboarding instead, and the message was gone before it could be read. Confirmed with a single `adb` back key, and in the system log: the screen's `BackHandler` was composed only while no restore was running, so switching it off during the attempt and on again after it **re-registered it above the keyboard's own back callback**. Back then skipped the keyboard. | (a) The handler stays registered for the life of the screen (until the restore is done), so its place in the order never changes. (b) Back asks the window whether the keyboard is open (`restoreBack`), and if it is, only closes it — the second layer, which holds whatever the order. (c) Tapping Restore closes the keyboard, so the answer above the words is visible. (d) **Behaviour change, deliberate:** back while a restore runs now stays put rather than falling through. | `Bug29_BackWithTheKeyboardOpenStaysOnRestoreTest` (instrumented; a real back key through the instrumentation, so the platform's callback choice is exercised): red 3/3 on the unfixed code, green 3/3 fixed. Its keyboard waits 1.5 s to settle first — without that the unfixed code passed one run in two, because the keyboard's callback landed on top by luck. `RestoreBackTest` (JVM) pins the decision. Mutation-swept: each layer alone keeps the device test green and both removed turn it red; removing (b) turns the JVM test red; (c) and (d) each turn their own test red. |
| **BUG30** — the recovery phrase outlived the screens it was typed on | Found in the same D10 run: back out of the restore screen, open it again, and the previous attempt's 24 words were still there. Onboarding, Recovery and restore are not navigation destinations; `MainActivity` swaps them on the vault's state, so their ViewModels belong to the **activity** and `onCleared` — which restore and "Back up now" relied on to forget the words — does not run while the app lives. Read across all three: a **successful Recovery kept the 24 correct words** on the heap for the life of the process; **onboarding kept the phrase it generated**, contradicting its own KDoc; restore kept them after the user left. "Back up now" is a navigation destination and was not affected. | Each screen forgets at the moment its words are finished with: restore on `RestoreEvent.Left`, which the screen sends from **both** ways out (back and the Back button) and which is ignored while a restore runs; Recovery when the outcome leaves the screen (`Unlocked`, `UpgradeBlocked`); onboarding when setup succeeds (`forgetThePhrase`). A **failure keeps the words** in every case, so a wrong word is fixable without retyping 24 (ADR-0011's bargain). **A limit, stated:** a JVM `String` cannot be wiped in place — "forget" means every reference dropped, which is all the app can honestly promise. | `Bug30_PhraseIsForgottenWhenItsScreenIsDoneTest` (JVM, 6 cases): each screen forgets on success or leaving (red on the unfixed code, all three), keeps on failure, and restore ignores leaving mid-attempt. `Bug29_…Test`'s leave cases assert the screen sends `Left`. Mutation-swept: never forgetting, forgetting on failure too, keeping the mnemonic, and not sending `Left` each turn a named test red. |
| **BUG31** — the nightly backup had no relationship to night | Found by the owner on 2026-09-23 after two nights of checking for an overnight backup and finding none. ADR-0027 scheduled a one-day `PeriodicWorkRequest`, and WorkManager counts a period from the moment the previous run **finished**, so the pass kept the hour of whatever run came last: on the owner's phone about 21:50. Nothing failed and nothing was recorded as failing — the pass was simply never due at night, while onboarding, More and "Back up now" all said "nightly". | (a) **Every pass first re-aims its successor at the next 03:00 local** (`BackupTime.nextAfter`) via `updateWork` with `setNextScheduleTimeOverride`, before it backs up, so a pass that dies still leaves the next one aimed; a failed re-aim costs one drifted night, not the schedule. (b) The schedule stays a periodic request, which cannot be lost the way a chain of one-time requests can. (c) **Migration:** a new unique name, `nightly-backup-0300`, enqueued with `KEEP`, and the old `nightly-backup` cancelled on every cold start — `UPDATE` would re-aim on each launch and push a pass Doze is still holding past 03:00 to tomorrow. (d) DST: a gap over 03:00 aims at its end; an overlap takes the first 03:00, so one night never writes two backups. (e) The screens keep "nightly" and name no hour; the hour is an aim, and Android decides (§5.9). | `Bug31_NightlyBackupStaysAnchoredTest` (Robolectric, real in-memory WorkManager, 6 cases: a fresh schedule aims at 03:00; a pass held until 06:40 aims its successor at 03:00 and keeps its constraint; a failed pass and a pass **stopped mid-backup** have both already re-aimed; opening the app while a pass is overdue does not push it to tomorrow; the drifting schedule is cancelled) and `BackupTimeTest` (JVM, 9 cases, DST against Europe/Helsinki because IST has none). Mutation-swept, 10 mutations, each red on its own set: aiming at itself at exactly 03:00; UTC for the device's zone; the later of two 03:00s; a gap skipping the night; never re-aiming; re-aiming after the backup instead of before (caught only by the stopped-mid-backup case, which was written after this mutation first survived); `UPDATE` for `KEEP`; not cancelling the old name; the re-aim dropping the battery constraint; no aim on the first schedule. |
| **BUG32** — CI ran no job for two weeks, and looked like it was merely red | Found on 2026-09-23 when the owner asked to check the run for `d440e9d`. `4238611` (2026-09-08, the APK size gate) wrote `printf '%s -> %s KiB\n'` into `ci.yml` with the `\n` as a **real newline**. The next line therefore began at column 0 and ended the `run: \|` block mid-script. GitHub does not load a workflow it cannot parse. The only sign is that each push's run is marked failed in zero seconds with **no jobs** (`run_started_at` equals `updated_at`). Every run from 2026-09-11 to 2026-09-23 looked like that, and the last runs that executed anything were on 2026-08-29. So "CI is red only on the receipt-corpus test" was never observed on GitHub, and the local `preMergeCheck` was the only gate. | (a) The newline is `\n` again. (b) `scripts/guard-workflows.sh` parses every `.github/workflows/*.yml` and checks its structure: a trigger, jobs, `runs-on`/`uses` per job, `run`/`uses` per step. It **fails when it cannot check** (no Python with PyYAML) rather than passing. (c) It is a **pre-push** guard (`CLAUDE.md` §11, §12), because a check inside `ci.yml` cannot run when `ci.yml` does not load. It also runs in the `guards` job, where it covers `release.yml`. (d) `CLAUDE.md` §11 now says to read a run's jobs, not only its colour. | The guard passes on the fixed tree (8 jobs, 1 job). On `ci.yml` as committed at `d440e9d` it fails at line 367 column 1, where GitHub's loader stopped. Across history, every committed `ci.yml` from `4238611` onwards fails the same parse, and `756398b` (2026-08-25), the version before it, passes. Found by checking a run's jobs rather than its colour. The same class of slip, a `\n` reinterpreted on its way into a file, recurred twice while this was being fixed, in scripted edits. Both times it was caught because the result was re-read rather than assumed. |
| **BUG33** — two CI gates could never pass: screenshots failed on font rendering, and the cacheDir ban caught itself | Found by the first CI run that executed jobs after BUG32 (2026-09-23, run 35858631887), read from its artifacts. **Screenshots:** 17 of 30 goldens failed on the Linux runner. Every image kept its size, at most 0.14% of pixels changed, and **no channel moved by more than 4/255**, which is anti-aliasing at glyph edges drawn by different fonts. The goldens are recorded on Windows. Roborazzi's default comparator (`maxDistance = 0.007` on channels scaled 0–1, read from its bytecode) fails a 2/255 change, so exact comparison could not pass there. **Static analysis:** the CI grep for `.cacheDir` (Law 5) matched only the root `build.gradle.kts`, whose `bannedApiCheck` task defines the regex `\.cacheDir\b`. The local task scans only `.kt` files under subprojects, so it never saw itself, and the step had failed on every run since `65a8450` (2026-08-14). | (a) `LfScreenshotOptions` (in `:core:designsystem`'s test sources, with a twin in `:core:ui`): `SimpleImageComparator(maxDistance = 0.055)`, which is 8/255 on R, G and B at once and twice the worst change seen. Roborazzi's default validator stays, so one pixel past the tolerance still fails the golden. It is passed at all four capture sites. (b) The CI step excludes the root `build.gradle.kts` by exact path (`:(top,exclude)`), for the reason docs and workflows are already excluded: it has to name the API to ban it. A module's build script is still covered. | `LfScreenshotOptionsTest` (6 cases, against the configured comparator and validator): `Bug33_antiAliasingNoiseAsCiSawIt_passes`, the bound itself passes, Roborazzi's default fails a 2/255 change, and a one-pixel shift, a palette change (#3B5BDB to #4263EB) and a single pixel past the tolerance each fail. Mutation-swept: 0.007 turns the two noise cases red, 0.1 the palette case, 2.0 the shift, palette and single-pixel cases, and a 1% validator the single-pixel case. **End to end through `verifyRoborazziSmsFullDebug`:** a golden noised on 4,170 edge pixels by 4/255 verifies, the same with 0.007 fails its screenshot test, and a golden shifted one pixel fails exactly that screenshot test. The grep is checked both ways: nothing matches as committed, and a `.cacheDir` added to `core/ui/build.gradle.kts` is caught. **A harness bug found on the way:** the sweep restored files by moving a backup back, which kept the backup's older timestamp. Kotlin's incremental compiler then kept the mutated 0.007 class, and a later run looked like a real failure. It was caught because a shift-only mutation turned tolerance cases red that never read a golden. The sweep now rewrites the original bytes, and the compiled constant is checked before a result is trusted. |
| **BUG34** — after a foreign QR code the scanner stopped delivering, with the camera still live | Found by reading `LfPhraseScanner` on 2026-09-26, while fixing BUG35. The analyser set `delivered = true` on the **first** code of any kind and never delivered again. A foreign code (a Wi-Fi sticker, a payment code) is answered `PhraseScan.KeepLooking` by all three screens, which leaves the scanner open. So a user who caught the wrong code first could then point at their real kit indefinitely and nothing would happen. | `ScanDeliveries` hands every **distinct** code to the screen once, and the same code seen in many frames is one delivery. The screen, not the scanner, decides what is ours: `PhraseQr` lives in `:core:domain`, which `:core:ui` does not see. Deliveries now run on the main thread. | `ScanFramesTest.Bug34_aForeignCodeFirst_theKitIsStillDelivered` (JVM), plus the same code repeated once and a code seen again after another. Mutation-swept: latching on the first code (the old behaviour) turns the Bug34 case and the "seen again" case red, and delivering every frame turns the repeat case red. **On the device (2026-09-26), with a real camera:** a test Wi-Fi QR shown on a screen left the camera open, and the Recovery Kit QR shown next (the public BIP-39 test phrase) closed it and filled all 24 words. |
...
| 0025 | How do backups get triggered, when the app never holds the phrase? | ✅ **Part 1 Accepted** — manual "Back up now" that asks for the 24 words each time, checked against the vault before sealing (§16 Q23). **Part 2 Proposed** — nightly backups sealed to a phrase-derived public key; not decided, not built |
| 0026 | How is a backup restored, and under which phrase? | ✅ **Accepted** — at first run only, from a link on onboarding's first screen; folder then list (or a single file); the backup is proven to open before anything is written; a fresh DEK wrapped under **the backup's own phrase**; never over an existing vault; a marker file written before the wrap routes an interrupted restore back to itself. Closes Q11 (§5.9, §7.3, §7.4) |
...
2. ~~**OCR language.**~~ **CLOSED — ADR-0021, on measurement.** The premise was the cost: "+~2 MB". Measured, Devanagari is **+0.61 MB**, because it reuses the same 10.55 MB `libmlkit_google_ocr_pipeline.so` that Latin already pays for and adds only `.tflite` assets. At that price the question stops being a budget question. **Both Latin and Devanagari ship**, on the owner's instruction at P4.

    **The rest of that instruction could not be honoured, and the reason is worth stating precisely.** Asked for "Devanagari + Kannada + Hindi + Malayalam", probed against `dl.google.com` rather than assumed: **ML Kit's models are per *script*, not per language.** *Hindi is written in Devanagari*, so the Devanagari model **is** Hindi support, and Marathi, Nepali, Sanskrit and Konkani come with it for the same 0.61 MB — there is no `text-recognition-hindi` artifact and nothing is missing. **Kannada and Malayalam have no ML Kit model at all**: the complete set is Latin, Chinese, Devanagari, Japanese and Korean. That is a capability limit, not a budget or configuration one, and the alternatives — a second engine for two scripts, or a cloud API that would violate Law 6's substantive half — are weighed and declined in ADR-0021.

    How much it costs is empirical: Indian retail receipts print the item block and the amounts in Latin almost universally, and §12's gate measures *item* recall, so the practical loss may be near zero. §12's diversity floor therefore gains a Kannada-bearing and a Malayalam-bearing receipt — not to be recognised, but to measure what is actually lost.
...
17. ~~**Two merchant-truncation defects, found by expanding the corpus at P2-9.**~~ **CLOSED — fixed on the owner's instruction.** Both were the same shape: a lazy merchant group ending on an alternation that a real payee can contain.

    - **`upi-debit-vpa`** ended on `\s*(?:\.|\bon\b|\bUPI\b|\bRef\b|$)`. The `\s*` can match *zero* characters, and `@` is a non-word character, so a VPA using the NPCI handle `@upi` put a word boundary immediately before it and the alternation fired **inside the address**: `kirana@upi` was captured as `kirana@`.
    - **`notification-upi-paid`** ended on `\s*(?:\.|\busing\b|\bfrom\b|$)`, so the first full stop inside a merchant name terminated it and `Dr. Lal PathLabs` became `Dr`.

    **The fix is that a terminator must be a separate token.** The word alternatives now require real whitespace in front of them — `\s+(?:on|UPI|Ref)\b` — so they can only end the merchant at a token boundary, never mid-address. For notifications a full stop now ends the merchant only at the end of the line (`\s*\.?\s*$`); mid-line the name runs until `using` or `from`, which is what every real payload in the corpus uses.

    **`upi-credit-vpa` had the identical tail and was fixed with them.** Q17 named only the debit rule; the credit twin would have truncated `someone@upi` on an incoming payment in exactly the same way. Fixing one and leaving its mirror is the half-fix that comes back.

    Verified against the **whole corpus before the edit**, by running the old and the new pattern over all 42 fixtures that these three rules claim: the two defect fixtures change and **nothing else moves**. `testdata/sms/upi-vpa-at-upi-handle-kept-whole` and `testdata/notifications/merchant-with-full-stop-kept-whole` are renamed from `-truncates`, because a fixture whose name says "truncates" while it asserts the opposite misleads the next reader. Restoring either old pattern turns `GoldenCorpusTest` red.

    **Two things the original entry expected that turned out not to be needed.** No ruleset version bump: `seedParserRules` deletes the shipped rules for the current version and re-inserts them from the asset on every seed, so an edited `v1.json` reaches every install on the next launch, and user-written rules are untouched by construction. And Q14's re-triage does not apply — it re-admits rows marked `SENDER_NOT_ALLOWLISTED`, and these messages were never rejected; they matched and produced a candidate.

    **So the fix is forward-looking, and that is a deliberate limit.** A message already parsed keeps the merchant it was parsed with: for an approved entry, re-parsing would rewrite committed history, and for a pending one the review screen already lets the user pick the right merchant before approving. A general "re-parse everything against the current ruleset" pass is a different mechanism, and it is not built.
...
20. ~~**A budget's period, start date and rollover cannot be changed after it is created.**~~ **CLOSED — built on the owner's instruction, immediately after the device verification that found it.** The editor showed one field because the write path offered one: `BudgetRepository` had `create`, `updateAmount` and `delete`, so `EditorForm` hid everything else behind `if (!editor.isEdit)` and a mis-ticked rollover could only be undone by deleting the budget and rebuilding it.

    **The fix was much smaller than the gap looked**, which is worth recording because it argues against deferring this class of thing. `openEditor` already loaded `period`, `startDate` and `rolloverEnabled` into the editor state, and the field-edit events already worked in edit mode — the state was complete and only the write was missing. No migration either: all four columns have existed since v10. `updateAmount` became `update(id, BudgetSettings)`, one statement over four columns, and the form's `if` now guards the category pair alone.

    **Applies immediately, chosen over three alternatives.** Deferring edits to the next period boundary was the most correct in principle and needed schema v11 for the pending values, plus a control that visibly does nothing today. A rollover-only edit dodged the period question entirely but left a wrong start date needing a rebuild. A "restart budget" action was honest about the period ending but accumulates binned rows. Applying now is what the user asked for read literally, and the case that actually happens — noticing a wrong setting soon after creating the budget — is precisely the case where the period in flight has nothing in it to disturb.

    One claim in the original entry was wrong and is corrected here: delete-and-rebuild did **not** lose "period history", because periods are derived arithmetic and there is no such record. It lost the start-date anchor if the user accepted the default, and the alert state, worth one duplicate notification.

21. ~~**`LfChip` conveys selection by colour alone.**~~ **CLOSED — fixed on the owner's instruction.** `LfChipStyle.Selected` changed the palette and set no semantics, so every chip in the app exposed the same accessibility state whether it was on or off. Found in a `uiautomator` dump of the budget editor, where all four period chips reported `selected="false"` while **Weekly** was visibly outlined.

    `Modifier.semantics { selected = ... }`, applied **only where the chip has an `onClick`**. A chip without one is a label, not a choice — the Recovery screen renders the twenty-four words as chips — and announcing "not selected" for each would be twenty-four reports of a state the thing does not have, on the one screen where every word matters.
...
22. ~~**A bill paid partly from a merchant wallet — which figure is the debit?**~~ **DECIDED (owner, 2026-09-19): the invoice value, and a wallet top-up is not spending.** The debit is what the purchase cost — ₹1,776.17 on the bigbasket invoice — so the entry and its items agree with the paper and reach item-grain analytics whole. A top-up of a merchant wallet is a transfer between the user's own balances; its SMS or notification arrives in the Inbox as a debit candidate and **is discarded there**, or it double-counts. The app has no "transfer" concept yet, so discarding is the user's step; a way to mark a candidate as a transfer is the natural follow-up and is not built. The original statement follows. **A bill paid partly from a merchant wallet — which figure is the debit?** Found on the owner's bigbasket invoice (private corpus, `2026-09-12-bigbasket-a4-pdf`): the invoice value is ₹1,776.17, of which ₹1,443.41 and ₹5.17 came from the bigbasket wallet and ₹327.60 was charged to RuPay. The page also prints `Final Total Rs.332.76` and `Payable Amount Rs. 0.00`. **The parser's answer is settled and is not this question:** it reads the printed invoice value, which is what the corpus fixture records, because that is what the paper says the purchase cost. The open question is the ledger's. If topping up the wallet was recorded as spending, booking ₹1,776.17 again double-counts ₹1,448.58; if it was not, booking only the card charge hides most of a grocery bill from item-grain analytics (ADR-0018). The same shape will arrive from Paytm, Amazon Pay and any cashback balance. Not decided unilaterally (CLAUDE.md §10: ledger semantics).

23. ~~**Nothing in the app writes or restores a backup, and the nightly worker §5.9 specifies cannot be built as written.**~~ **CLOSED — ADR-0025, on the owner's choice of option A.** "Back up now" in More asks for the 24 words each time, checks them against *this* vault (`DekManager.verifyPhrase`) before sealing anything, writes a verified `.lfbk` and the receipt images into the chosen SAF folder, keeps the newest five, and records `lastBackupAt` only after verification. Option C — nightly backups sealed to a phrase-derived public key — is drafted in ADR-0025 part 2 as *proposed*. Restore was then still §16 Q11's gap; **closed by ADR-0026**. The original statement follows.

    **Nothing in the app writes or restores a backup, and the nightly worker §5.9 specifies cannot be built as written.** `DatabaseBackupManager.writeBackup` and `restore` have no production caller — only `BackupRestoreRoundTripTest`. There is no `BackupWorker`, no "Back up now", no restore screen; onboarding takes the SAF tree grant and records `backupTreeUri`, and nothing has ever written to it. **This is not an oversight that can be closed by wiring it up.** A `.lfbk` is phrase-derived (ADR-0011) and the app never holds the phrase after onboarding, which is the same wall ADR-0019 hit for the pre-migration snapshot: a scheduled job has no way to obtain the key. §5.9's nightly `PeriodicWorkRequest` and BUG4(c)'s "`lastBackupAt` age > 7 days" banner both assume a trigger that does not exist, so **the app currently promises a durability it does not have** — the one thing §7 and ADR-0019 both say never to do.
