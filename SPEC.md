...
21. **`LfChip` conveys selection by colour alone.** A selected chip differs from an unselected one only in its palette (`LfChipStyle.Selected`); nothing sets the `selected` semantics property, so every chip in the app exposes the same accessibility state whether it is on or off. Confirmed in a `uiautomator` dump of the budget editor, where all four period chips reported `selected="false"` while **Weekly** was visibly outlined.

    It affects every chip row shipped: the analytics range chips, the ten filter rows, the budget category, subcategory and period pickers. A screen reader user cannot tell which range is active or which categories a filtered figure excludes — and §5.6 puts the filter count on the button precisely because a screen quietly showing a subset is how someone concludes their data is missing. That reasoning does not survive the state being invisible to the one user who cannot see the colour.

    Same family as the rollover switch's missing name, and about as small: `selected` in the chip's semantics, set only where `onClick != null` so the Recovery screen's non-selectable word list does not start announcing "not selected" at every word. Recorded rather than built because `LfChip` was not part of the work that found it (§9.6).
